...
export default (options: Object) => {
  const {
    fetchOneActions: [FETCH_ONE, FETCH_ONE_SUCCESS, FETCH_ONE_FAILURE] = ["1", "1s", "1f"],
    fetchAllActions: [FETCH_ALL, FETCH_ALL_SUCCESS, FETCH_ALL_FAILURE] = ["2", "2s", "2f"],
    deleteActions: [DELETE_ONE, DELETE_ONE_SUCCESS, DELETE_ONE_FAILURE] = ["3", "3s", "3f"],
    updateActions: [UPDATE_ONE, UPDATE_ONE_SUCCESS, UPDATE_ONE_FAILURE] = ["4", "4s", "4f"],
    createActions: [CREATE_ONE, CREATE_ONE_SUCCESS, CREATE_ONE_FAILURE] = ["5", "5s", "5f"],
    reducer
  } = options
  ...
  function __entityReducer__(state: Object = {}, action: Object) {
    ...
    switch (type) {
      case FETCH_ONE: {
        ...
        return mergeNewEntities(state, [{ id }], ENTITY_STATUS_LOADING)
      }
      ...
      case FETCH_ONE_FAILURE: {
        ...
        return mergeNewEntities(state, [{ id, __message__: message }], ENTITY_STATUS_DATA_UNAVAILABLE)
      }
      case CREATE_ONE: {
        return state
      }
      case CREATE_ONE_SUCCESS: {
        return mergeNewEntities(state, [action.payload], ENTITY_STATUS_DATA_AVAILABLE)
      }
      ...
      case DELETE_ONE_SUCCESS: {
        ...
        return replaceNewEntities(state, [{ id }], ENTITY_STATUS_DATA_UNAVAILABLE)
      }
